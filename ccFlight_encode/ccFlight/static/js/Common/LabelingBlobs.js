...
//label是srcLab的指代，srcLab是一张图片的像素总长
//curLabCnt初始是0，后来会增长数量
            ...
            // console.log(imgData.groupVec)
            index++;
        }
    }
    var labeldata = []
    for (let index = 0; index < imgData.label.length; index++) {
        const element = imgData.label[index];
        if (element != -1) {
